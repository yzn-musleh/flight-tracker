"""Characterizes flight_api.get_flight_status()'s retry/backoff behavior for
429/5xx responses, and that a non-429 4xx fails fast without retrying. No
real sleeping (resilience's retry_with_backoff is exercised through its
real, injectable sleep -- here monkeypatched to a no-op so the test suite
stays fast)."""
...
def test_network_error_raises_lookup_error(monkeypatch):
    import requests
